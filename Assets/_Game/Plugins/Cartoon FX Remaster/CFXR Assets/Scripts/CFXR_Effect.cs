...
namespace CartoonFX
{
	...
	public partial class CFXR_Effect : MonoBehaviour
	{
...
#if !DISABLE_CAMERA_SHAKE
			if (cameraShake != null && cameraShake.isEnabled)
			{
		...
		void Awake()
		{
			...
			if (!GetLightsUseLinearIntensityCompat() && animatedLights != null)
			{
				foreach (var animLight in animatedLights)
				{
...
#endif
		static bool GetLightsUseLinearIntensityCompat()
		{
			try
			{
				var prop = typeof(GraphicsSettings).GetProperty("lightsUseLinearIntensity", BindingFlags.Public | BindingFlags.Static);
...
#if !DISABLE_CAMERA_SHAKE
			if (cameraShake != null && cameraShake.isEnabled && !GlobalDisableCameraShake)
			{
#if UNITY_EDITOR
		...
		void onEditorUpdate()
		{
			...
			{
#if !DISABLE_CAMERA_SHAKE
				if (cameraShake != null && cameraShake.isEnabled && parentParticle.time < particleTime && parentParticle.time < 0.05f)
				{
